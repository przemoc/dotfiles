...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({ 01, 02, 03, 04, 05, 06, 07, 08, 09, 10 }, s, layouts[2])
end
-- }}}
...
-- {{{ User functions
ssh_host_completion = function(cmd, cur_pos, ncomp)
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey, "Shift"   }, "s",
              function ()
                  awful.prompt.run({ prompt = "ssh: " },
                  mypromptbox[mouse.screen].widget,
                  function(h) awful.util.spawn(terminal_latin2 .. " ssh " .. h) end,
                  ssh_host_completion,
                  awful.util.getdir("cache") .. "/ssh_history")
              end),
    awful.key({ modkey,           }, "v",
              function ()
                  awful.prompt.run({ prompt = "rdesktop: " },
                  mypromptbox[mouse.screen].widget,
                  function(h) awful.util.spawn("rdesktop -k pl -g 1920x1058 " .. h) end,
                  ssh_host_completion,
                  awful.util.getdir("cache") .. "/rdesktop_history")
              end)
)
...
function add_calendar(inc_offset)
	...
	local cal = awful.util.pread("ncal -M -Ch -m " .. datespec)
	cal = string.gsub(cal, "%s*$", "")
	cal = string.gsub(cal, "^(.-)\n", "<span background=\"black\">%1</span>\n")
...
-- switch month using mouse wheel
